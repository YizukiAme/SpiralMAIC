...
describe('access-code proxy', () => {
  ...
  test('keeps the workbench server-side gated by both feature flags', () => {
    vi.stubEnv('ACCESS_CODE', '');
    vi.stubEnv('NEXT_PUBLIC_PRO_WORKBENCH_ENABLED', 'true');
    vi.stubEnv('OPENMAIC_AGENT_RUNTIME_ENABLED', 'true');
    vi.stubEnv('DATABASE_URL', '');
    expect(proxy(new NextRequest('http://localhost/workbench')).status).toBe(404);

    vi.stubEnv('DATABASE_URL', 'postgres://localhost/openmaic');
    expect(proxy(new NextRequest('http://localhost/workbench')).status).toBe(200);
  });
