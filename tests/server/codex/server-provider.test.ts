...
describe('getCodexNativeServerProvider', () => {
  ...
  it('publishes fixed image metadata across a same-account token rotation', async () => {
    const tokenProvider = {
      getValidCredentials: vi
        ...
        .mockResolvedValue({
          accessToken: 'account-a-rotated-secret',
          accountId: 'account-a-secret',
        }),
    };
    ...
    await expect(getCodexNativeImageProvider()).resolves.toEqual({
      models: ['gpt-image-2'],
    });
    expect(tokenProvider.getValidCredentials).toHaveBeenCalledTimes(2);
  });
