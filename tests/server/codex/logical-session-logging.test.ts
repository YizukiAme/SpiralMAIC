...
describe('Codex logical session logging hygiene', () => {
  ...
  it('does not pass the raw revisit attempt identity to logger calls', () => {
    const unsafe = loggerArguments('app/classroom/[id]/revisit/page.tsx').filter((argumentsText) =>
      /\battemptId\b/.test(argumentsText),
    );
