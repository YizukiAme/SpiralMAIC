...
describe('Codex image response safety', () => {
  ...
  it('publishes a valid image after a same-account token rotation', async () => {
  ...
  it('preserves a classified error after a same-account token rotation', async () => {
    let current = { accessToken: 'account-a-token', accountId: 'account-a' };
    const credentials = {
      getValidCredentials: vi.fn(async () => ({ ...current })),
    } satisfies CodexTokenProvider;
    ...
    expect(caught).toMatchObject({ code: 'FORBIDDEN', upstreamStatus: 403 });
