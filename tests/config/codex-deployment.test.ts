...
describe('Codex OAuth deployment contract', () => {
  it('documents always-on browser and device login without feature flags', () => {
    ...
    expect(env).not.toContain('OPENMAIC_ENABLE_CODEX_OAUTH');
    expect(env).not.toContain('OPENMAIC_CODEX_BROWSER_LOGIN');
    expect(env).not.toMatch(/device[^\n]{0,80}fallback/i);
    expect(env).not.toMatch(/fallback[^\n]{0,80}device/i);
