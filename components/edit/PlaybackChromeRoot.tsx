...
export const PlaybackChromeRoot = forwardRef<PlaybackChromeRootHandle, PlaybackChromeRootProps>(
  function PlaybackChromeRoot(
    ...
    useEffect(() => {
      // Revisit uses a transcript-only shell with no live ChatArea to drain.
      // Keep its synchronous setup isolated while the regular classroom path
      // below awaits in-flight presentation actions before sharing whiteboard state.
      if (revisitConfig) {
        ...
        const engine = new PlaybackEngine([currentScene], actionEngine, audioPlayerRef.current, {
          ...
          onProgress: (snapshot) => {
            // Revisit stages are transient; keep their resume state session-only.
    ...
    const showElementReference =
      !revisitConfig && piChatEnabled && coursewareReferenceEnabled && mode === 'playback';
          ...
          {mode === 'playback' && (
            ...
            >
              <Roundtable
                ...
                isSoftClosing={revisitConfig ? false : chatIsSoftClosing}
