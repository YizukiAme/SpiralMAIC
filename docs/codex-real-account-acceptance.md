...
Codex sign-in is available after installation with no enablement flags. Settings presents **Sign in
with ChatGPT** first and **Use device code** alongside it. This local-process pass begins with browser
PKCE; the device-code method is exercised separately below.
...
Set `ACCESS_CODE` in `.env.local`. The Compose deployment must remain one container with the
`openmaic-data` named volume mounted at `/app/data`:
...
Select **Use device code** directly in Settings, complete login manually, and run the normal harness
against the published base URL. Restart without removing the named volume:
