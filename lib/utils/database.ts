    ...
    if (data.chatSessions !== undefined) {
      ...
      const rollbackImage = await db.transaction(
        ...
        async () => ({
          ...
          overtimeExtensions: (
            await db.overtimeExtensions.bulkGet(
              (data.overtimeExtensions ?? []).map((extension) => extension.id),
            )
          ).filter((extension): extension is OvertimeExtension => extension !== undefined),
        }),
      );
      const rollbackRows = () =>
        db.transaction(
          ...
          async () => {
            ...
            await db.overtimeExtensions.bulkDelete(
              (data.overtimeExtensions ?? []).map((extension) => extension.id),
            );
            ...
            await db.overtimeExtensions.bulkPut(rollbackImage.overtimeExtensions);
          },
        );
