...
export async function loadStageData(stageId: string): Promise<StageStoreData | null> {
  try {
    ...
    return {
      // DocumentStore owns the universal DSL shape; Spiral stores its additive
      // roster metadata on the same JSON object and restores the app-level type
      // at this boundary.
      stage: document.stage as Stage,
        ...
        withRuntimeStorageExclusiveLockUntilSettled(async (releaseCaller) => {
          try {
            ...
            await db.transaction(
              'rw',
              [db.stages, db.scenes, db.stageOutlines, db.agentEditSessions, db.overtimeExtensions],
              async () => {
            ...
            // deleted, so optional cleanup failure must not misreport the cascade.
            try {
