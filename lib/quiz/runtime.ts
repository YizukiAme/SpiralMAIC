...
async function withAttemptLock<T>(attemptId: string, work: () => Promise<T>): Promise<T> {
  // Web Locks are a browser coordination primitive. Newer Node releases also
  // expose navigator.locks, but using that server-global lock would serialize
  // independent RuntimeStore clients and change the intended lock-free path.
